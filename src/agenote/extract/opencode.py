...
adapter 只提供差异：db 路径、source 名、categorize 启发式。
"""
...
@register("opencode")
def extract_opencode() -> tuple[list, list[str]]:
...
def trace_session(session_id: str) -> dict:
    ...
    与 extract_opencode 的区别：
    - 不截断（[:1000]/[:2000] 全去掉），reasoning/tool/patch 取全文
    - 返回结构化 {source, session, messages: [{role, ts, parts: [{type, text}]}]}，
      而非拼成单条 content
...
SOURCES["opencode"].trace = trace_session
