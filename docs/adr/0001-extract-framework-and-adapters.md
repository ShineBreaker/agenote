...
user→assistant 配对 + 构造，事实型源（Hermes SQLite facts，无 turn 概念）直接产出。配对逻辑集中到 framework 一处。

## Considered Options

- **adapter 产 turn，framework 统一配对**：被拒。当时 Hermes `memory_store.db/facts` 是 1:1 映射、无 turn 概念，
...
- `extract_hermes` 曾在 2026-08-14 从 `reconcile.py` 迁到 `extract/hermes.py`；2026-09-24 该旧 adapter 被删除。
  它读取的是 `memory_store.db/facts`；Hermes 内置 `MEMORY.md` / `USER.md` 则一直由独立的
  `scan-memories` 只读扫描，两条路径读取的不是同一存储。`ReconciledFact` 早于退役抽到
  `extract/models.py`，解除 extract→reconcile 的反向依赖（extract 子包自此自洽）。
...
- **2026-08-14 阶段 1a/1b/1c 全部完成**：当时 7 源全部 @register 进 `extract.base.SOURCES`；
  `pi.py` 已删除且从未注册；`extract_hermes` 迁至 `extract/hermes.py`（`_extract_hermes_title`
